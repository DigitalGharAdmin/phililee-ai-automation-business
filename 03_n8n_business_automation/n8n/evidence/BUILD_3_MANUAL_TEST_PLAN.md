# Build 3 manual test plan — NOT EXECUTED
...
| B. AI fallback | Enable AI only; use unavailable credential or a controlled timeout/malformed response fixture | fallback, deterministic route preserved, logging succeeds if Sheets is healthy, email remains disabled |
...
| D. Gmail before send | In isolated test copy with explicit approval, make Gmail credential unavailable after confirmed sending marker | One attempt; unknown/needs_reconciliation; no completed claim; inspect account to confirm no actual send |
...
Build 2 live evidence remains separate; Build 3 live acceptance is pending.
